        ...
        if (request.url.toString().contains(exManga.replace("api.", "")) and !(request.url.toString().contains("/auth/login"))) {
            if (exManga_access_token.isEmpty() and emailEX.isNotEmpty() and passwordEX.isNotEmpty()) {
                val response = client.newCall(
                    POST(
                        "$exManga/auth/login",
                        body = """{"email":"$emailEX", "password":"$passwordEX"}""".toRequestBody("application/json; charset=utf-8".toMediaTypeOrNull()),
                        headers = exHeaders(),
                    ),
                ).execute()
                exManga_access_token = json.decodeFromString<ExAuthDto>(response.body.string()).data.orEmpty()
            }
        ...
        network.cloudflareClient.newBuilder()
            ...
            .addInterceptor { chain ->
                ...
                if (originalRequest.url.toString().contains(exManga) and !response.isSuccessful) {
                    var errorText = when (originalRequest.url.toString()) {
                        "$exManga/auth/login" -> json.decodeFromString<ExAuthDto>(response.body.string()).message.orEmpty()
                        else -> json.decodeFromString<ExWrapperDto<String>>(response.body.string()).message.orEmpty()
                    }
                    if (errorText.isEmpty()) { errorText = "Нет сообщения об ошибке" }
